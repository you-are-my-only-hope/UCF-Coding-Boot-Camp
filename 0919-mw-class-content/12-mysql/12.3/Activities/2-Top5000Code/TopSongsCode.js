...
var artistSearch = function() {
    inquirer.prompt({
        ...
        var query = "SELECT * FROM top5000 WHERE artist LIKE ?";
...
var multiSearch = function() {
    //TODO
    var query = 'SELECT `artist`, count(*) as total FROM top5000 GROUP BY artist HAVING count(`artist`) > 1 order by total desc';
...
var songSearch = function() {
    inquirer.prompt({
        ...
        //TODO
        var sql = 'SELECT * FROM top5000 WHERE song LIKE ?';
        connection.query(sql, [answer.song], function(err, res) {
